...
const PromoVideo: React.FC = () => {
  ...
  const checkKey = async () => {
    // Usamos (window as any) para evitar errores de compilación en Vercel
        ...
        <div className="w-full h-full flex items-center justify-center bg-black/80 border border-[#FF0000]/30 rounded-xl p-6 backdrop-blur-sm">
           <div className="text-center">
              ...
              <button 
                ...
                className="bg-[#FF0000] hover:bg-[#FF00FF] text-white px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-widest transition-colors cursor-pointer"
              >
                 Habilitar Video
              </button>
    ...
    <div className="relative w-full aspect-video rounded-xl overflow-hidden border border-white/10 shadow-[0_0_30px_rgba(255,0,0,0.2)] bg-black">
      ...
      {error && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/90">
           ...
           <button onClick={generatePromoVideo} className="mt-4 text-xs underline text-white cursor-pointer">Reintentar</button>
        </div>
      )}
