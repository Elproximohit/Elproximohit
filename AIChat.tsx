...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: '¿Necesitas detalles sobre el Sorteo o la Plantilla de Pro Tools? Puedo ayudarte.' }
  ]);
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 flex flex-col items-end pointer-events-auto">
          ...
          >
            ...
            <div className="bg-gradient-to-r from-[#8B0000] to-black p-4 flex justify-between items-center border-b border-[#FF0000]/20">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-heading font-bold text-white tracking-wider">IA HIT MAKER</h3>
              </div>
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="bg-white/10 p-3 rounded-lg rounded-tl-none flex gap-1">
                    <span className="w-1.5 h-1.5 bg-[#FF00FF] rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                    <span className="w-1.5 h-1.5 bg-[#FF00FF] rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                    <span className="w-1.5 h-1.5 bg-[#FF00FF] rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-3 border-t border-white/10 bg-black">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Pregunta sobre la plantilla..."
                  className="flex-1 bg-transparent text-white placeholder-white/30 text-sm focus:outline-none"
                />
                <button
                  onClick={handleSend}
                  disabled={isLoading || !input.trim()}
                  className="bg-[#FF0000] p-2 rounded-lg hover:bg-[#DC143C] transition-colors disabled:opacity-50"
                  data-hover="true"
                >
      ...
      <motion.button
        ...
        className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-tr from-[#FF0000] to-[#FF00FF] flex items-center justify-center shadow-lg shadow-[#FF0000]/40 border border-white/20 z-50 group"
        data-hover="true"
      >
        {isOpen ? (
